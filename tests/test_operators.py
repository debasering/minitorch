...
def test_sigmoid(a):
    ...
    * it is  strictly increasing.
    ...
    assert sigmoid(a - 100) < sigmoid(a)
...
def test_distribute():
    ...
    """
    assert mul(5, add(10, 8)) == add(mul(5, 8), mul(5, 10))
...
def test_other():
    ...
    """
    assert mul(5, mul(7, 10)) == mul(mul(5, 7), 10)
    assert add(5, add(7, 10)) == add(add(5, 7), 10)

    assert mul(-5, -10) == mul(5, 10) == 50
    assert add(10, -10) == 0
    assert mul(10, 0) == 0
...
def test_sum_distribute(ls1, ls2):
    ...
    """
    assert addLists([3, 4, 5], [8, 20, 9]) == [11, 24, 14]
    assert addLists([], []) == []
    assert addLists([1], [10]) == [11]
